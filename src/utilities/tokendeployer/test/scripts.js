...
import { Cell, beginCell, Dictionary, Address } from '@ton/core';
...
deployButton.addEventListener('click', async () => {
    ...
    data: beginCell()
            .storeCoins(0)
            .storeAddress(Address.parse(recipientAddress)) // Parse the string to an Address object
            .storeRef(jettonContent)
            .storeRef(Cell.fromBoc(Buffer.from(jettonWalletCodeBOC, 'base64'))[0])
            .endCell(),
    };

    const deployAddress = tonConnectUI.account?.address; // Get the connected wallet's address
    if (!deployAddress) {
    ...
    let feeAmountRaw = BigInt(0); // Declare feeAmountRaw here
    ...
    // Handle Fee
    if (feePercentageRadio.checked) {
        feeAmountRaw = BigInt(Math.floor(parseFloat(initialSupplyInput.value) * PERCENTAGE_FEE / 100) * (10 ** DECIMALS));
        const feeRecipientAddress = deployAddress;

        const feeMintBody = beginCell()
            .storeUint(1, 32)
            .storeUint(1, 64)
            .storeAddress(Address.parse(feeRecipientAddress))
            .storeCoins(feeAmountRaw)
            .storeRef(beginCell().endCell())
            .endCell();

        messages.push({
            address: deployAddress, // Send to the minter contract
